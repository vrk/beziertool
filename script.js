...
window.onload = function() {
	...
	JSButton.addEventListener("click", function() {
		...
		if (gBezierPath) {
			...
			var codeBox = document.getElementById('putCode');
			codeBox.innerHTML = gBezierPath.toCodeString();
		}
	}, false);

	var P3Button = document.getElementById('Processing');
	P3Button.addEventListener("click", function() {
		gLanguage = Language.Processing;
		if (gBezierPath) {
			gBezierPath.draw(gBackCtx);
			var codeBox = document.getElementById('putCode');
			codeBox.innerHTML = gBezierPath.toCodeString();
...
function Point(newX, newY) {
	...
	this.contains = function(pt) {
		var xInRange = pt.x() >= xVal - SELECT_RADIUS
				&& pt.x() <= xVal + SELECT_RADIUS;
		var yInRange = pt.y() >= yVal - SELECT_RADIUS
				&& pt.y() <= yVal + SELECT_RADIUS;
...
function LineSegment(pt, prev) {
	...
	this.toJSString = function() {
		if (!my.prev)
			return '  ctx.moveTo(' + Math.round(my.pt.x()) + ' + xoff, '
					+ Math.round(my.pt.y()) + ' + yoff);';
		else {
			var ctrlPt1x = 0;
			var ctrlPt1y = 0;
			var ctrlPt2x = 0;
			...
			return '  ctx.bezierCurveTo(' + ctrlPt1x + ' + xoff, ' + ctrlPt1y
					+ ' + yoff, ' + ctrlPt2x + ' + xoff, ' + ctrlPt2y
					+ ' + yoff, ' + x + ' + xoff, ' + y + ' + yoff);';
		}
	}
	this.toP3String = function() {
		if (!my.prev)
			return '  vertex(' + Math.round(my.pt.x()) + ' + xoff, '
					+ Math.round(my.pt.y()) + ' + yoff);';
		else {
			var ctrlPt1x = 0;
			var ctrlPt1y = 0;
			...
			return '  bezierVertex(' + ctrlPt1x + ' + xoff, ' + ctrlPt1y
					+ ' + yoff, ' + ctrlPt2x + ' + xoff, ' + ctrlPt2y
					+ ' + yoff, ' + x + ' + xoff, ' + y + ' + yoff);';
		}
	}
	...
	function drawCurve(ctx, startPt, endPt, ctrlPt1, ctrlPt2) {
		...
		ctx.beginPath();
		ctx.moveTo(startPt.x(), startPt.y());
		ctx.bezierCurveTo(ctrlPt1.x(), ctrlPt1.y(), ctrlPt2.x(), ctrlPt2.y(),
				endPt.x(), endPt.y());
		ctx.stroke();
		ctx.restore();
	}
...
function BezierPath(startPoint) {
	...
	this.toJSString = function() {
		var myString = [ 'function drawShape(ctx, xoff, yoff) {',
				'  ctx.beginPath();', ];
	...
	this.toP3String = function() {
		var myString = [ 'noFill();', 'beginShape();', ];

		var current = my.head;
		while (current != null) {
			myString.push(current.toP3String());
			current = current.next;
		}
		myString.push('endShape();');
		return myString.join('\n');
	}
